...
class AbstractDataset(Dataset):
    ...
    def __init__(self, path, 
                ...
                post_funcs = None,
                raw_img_data_root = None,
                edited_img_data_root = None,
                ) -> None:
        super().__init__()
        self.raw_img_data_root = raw_img_data_root
        self.edited_img_data_root = edited_img_data_root
    ...
    def __getitem__(self, index, num_trials=3):
        ...
        for i in range(num_trials):
            try:
                ...
                #     tp_img = self.img_loader(tp_path)
                #     print(f"!!![IMPORTANT] image truncated at {tp_path}")
                #     ImageFile.LOAD_TRUNCATED_IMAGES = False
                #tp_img = self.img_loader(tp_path)
                # shape, here is PIL Image
                tp_shape = tp_img.size

                # print("loaded image time {}".format(time.time() - start))
                ####################################################
                # My Modification
                ...
                # start = time.time()
                if gt_path == 'raw' or gt_path == 'text2img':
                    if gt_path == 'raw':
                        # All black
                        ...
                        label = 0
                        # print(f"{tp_path} 得到全黑")
                    elif gt_path == 'text2img':
                        # All white
                        temp = np.array(tp_img)
                        gt_img = np.ones_like(temp)*255
                        gt_shape = (temp.shape[1], temp.shape[0])
                        label = 1
                        # print(f"{tp_path} 得到全白")
                # Original
                # if "negative" then gt is a image with all 0
                    ...
                    if gt_path != "Negative":
                        ...
                        #     ImageFile.LOAD_TRUNCATED_IMAGES = True
                        #     gt_img = self.img_loader(gt_path)
                        #     print(f"!!![IMPORTANT] image truncated at {gt_path}")
                        #     ImageFile.LOAD_TRUNCATED_IMAGES = False
                        gt_img = self.img_loader(gt_path)
                        gt_shape = gt_img.size
                        label = 1
                        # print(f"{tp_path} 正常读取mask在{gt_path}")
                    else:
                        temp = np.array(tp_img)
                        gt_img = np.zeros((temp.shape[0], temp.shape[1], 3))
                        gt_shape = (temp.shape[1], temp.shape[0])
                ...
                # print("get mask time {}".format(time.time() - start))
                ...
                # print("common transform time {}".format(time.time() - start))
                # Do post-transform (paddings or resizing)    
                # start = time.time()
                res_dict = self.post_transform(image = tp_img, masks = masks_list)
                # print("post transform time {}".format(time.time() - start))
                ...
                # data_dict['index'] = index # forward里没这个
                ...
                # ====================================
                # print("post process time {}".format(time.time() - start))
                break
            except Exception as e:
                print("Try round: {}".format(i), index, gt_path, e)
                index = random.randint(0, len(self.tp_path) - 1)
