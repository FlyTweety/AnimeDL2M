...
class PixelAUC(AbstractEvaluator):
    ...
    def Cal_AUC(self, y_true, y_scores, shape_mask=None):
        # 输入校验
        if not (isinstance(y_true, torch.Tensor) and isinstance(y_scores, torch.Tensor)):
            raise TypeError("y_true and y_scores must be torch.Tensor")
        if y_true.shape != y_scores.shape:
            raise ValueError("y_true and y_scores must have the same shape")
        #if not torch.all(torch.isin(y_true, torch.tensor([0, 1]))):
        #    raise ValueError("y_true must contain only 0s and 1s")
        
        if shape_mask is not None:
            if shape_mask.shape != y_true.shape:
                raise ValueError("shape_mask must match y_true/y_scores shape")
            #if shape_mask.dtype != torch.bool:
            #    raise ValueError("shape_mask must be a boolean tensor")
            # 直接过滤有效区域
            valid_mask = shape_mask.flatten() > 0
            y_true = y_true.flatten()[valid_mask]
            y_scores = y_scores.flatten()[valid_mask]
        else:
        ...
        #return ratio
        auc = torch.trapz(tpr, fpr)  # 依赖fpr单调递增
        return auc.item()
    ...
    def batch_update(self, predict, mask, shape_mask=None, *args, **kwargs):
        ...
        if self.mode == "origin":
            for idx in range(predict.shape[0]):
                ...
                auc = self.Cal_AUC(mask[idx], predict[idx], single_shape_mask)
                AUC_list.append(auc)
        elif self.mode == "reverse":
            for idx in range(predict.shape[0]):
                single_shape_mask = None if shape_mask is None else shape_mask[idx]
                auc = self.Cal_AUC(mask[idx], 1 - predict[idx], single_shape_mask)
                AUC_list.append(auc)
        elif self.mode == "double":
            for idx in range(predict.shape[0]):
                single_shape_mask = None if shape_mask is None else shape_mask[idx]
                auc_origin = self.Cal_AUC(mask[idx], predict[idx], single_shape_mask)
                auc_reverse = self.Cal_AUC(mask[idx], 1 - predict[idx], single_shape_mask)
                AUC_list.append(max(auc_origin, auc_reverse))
        else:
    ...
    def recovery(self):
        return None
class PixelAUC_bug(AbstractEvaluator):
    def __init__(self, threshold=0.5, mode="origin") -> None:
        self.name = "pixel-level AUC"
        self.desc = "pixel-level AUC"
        self.threshold = threshold
    ...
    def Cal_AUC(self, y_true, y_scores, shape_mask=None):
        ...
        if shape_mask is not None:
            valid_mask = shape_mask.bool().flatten()  # 确保是布尔类型
            y_true = y_true.flatten()[valid_mask]
            y_scores = y_scores.flatten()[valid_mask]
...
if __name__ == "__main__":
    ...
    # 测试案例1：二维输入
    PixelAUCINSTANCE = PixelAUC()

    y_true = torch.tensor([[1, 0], [0, 1]], device="cuda").float()
    y_score = torch.tensor([[0.8, 0.2], [0.4, 0.6]], device="cuda")
    print(PixelAUCINSTANCE.Cal_AUC(y_true, y_score, None))  # 应输出 1.0

    # 测试案例2：相同分数
    y_true = torch.tensor([1, 0, 1, 0], device="cuda")
    y_score = torch.tensor([0.5, 0.5, 0.5, 0.5], device="cuda")
    print(PixelAUCINSTANCE.Cal_AUC(y_true, y_score, None))  # 应输出 0.5
