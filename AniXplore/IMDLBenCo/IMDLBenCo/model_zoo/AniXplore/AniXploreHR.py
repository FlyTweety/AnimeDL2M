...
class MixVisionTransformer(nn.Module):
    ...
    def stage1(self, B, x):
        ...
        for i, blk in enumerate(self.block1):
            x = blk(x, H, W) 
        x = self.norm1(x)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous() # reshape to (B, C, H, W)
        return x
...
class AniXploreHR(nn.Module):
    def __init__(self, seg_pretrain_path, conv_pretrain=False, freeze_segmodel=False, image_size=1024):
        super(AniXploreHR, self).__init__()
        self.convnext = ConvNeXt(conv_pretrain)
        self.segformer = MixVisionTransformer(seg_pretrain_path, img_size=1024)
