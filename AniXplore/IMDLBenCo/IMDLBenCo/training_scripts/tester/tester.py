...
def test_one_loader(model: torch.nn.Module,
    ...
    # save_dir = f'/home/fmg/chenyang/IMDLBenCo/output_debug/{formatted_time}'
    # print(f"\n*********\n保存到{save_dir}\n\n")
    ...
    for data_iter_step, data_dict in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # print(f"len(data_dict['mask']) = {len(data_dict['mask'])}, len(output_dict['pred_mask']) = {len(output_dict['pred_mask'])}, data_dict['name'] = {data_dict['name']}")
        # for i, (image, pred_mask, mask) in enumerate(zip(data_dict['image'], output_dict['pred_mask'], data_dict['mask'])):
        #     #print(i, len(data_dict['image']), len(output_dict['pred_mask']))
        #     #print(i, data_iter_step,data_dict['name'])
        #     name = data_dict['name'][0].split(".")[0]
        #     print(f"保存{name}")

        #     save_image(image, os.path.join(save_dir, f'{name}.jpg'))
        #     save_image(pred_mask * 1.0, os.path.join(save_dir, f'{name}_pred_mask.png'))
        #     save_image((pred_mask > 0.5) * 1.0, os.path.join(save_dir, f'{name}_pred_mask_threshold_0.5.png'))
        #     save_image(mask, os.path.join(save_dir, f'{name}_mask.png'))
...
def test_one_epoch(model: torch.nn.Module,
    ...
    with torch.no_grad():
        ...
        #with open('/home/fmg/chenyang/workspace/log/test_trufor/my_debug_test.txt', 'w') as f:
            #f.write(f"{type(output_dict)}, {output_dict.keys()}, {data_dict.keys()}")
