...
def get_albu_transforms(type_ = 'train', output_size = (1024, 1024)):
    ...
    assert type_ in ['train', 'test', 'pad', 'resize', "test_extra_noise"] , "type_ must be 'train' or 'test' of 'pad' "
    trans = None
    if type_ == 'train':
        trans = albu.Compose([
            # Rescale the input image by a random factor between 0.8 and 1.2
            albu.RandomScale(scale_limit=(-0.2, 0.2), p=0.5),
    ...
    if type_ == 'test_extra_noise':
        ...
        albu.GaussianBlur(
            blur_limit=(3, 7),  # kernel size 必须为奇数
            p=1
        ),
        ])
